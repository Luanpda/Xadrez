...
export function movimentoPadraoCaptura(cell,pecaCliclada,isBranco,classesPecas,pecaBranco,pecaPreta,nome) {
    const imgInicial = pecaCliclada.querySelector('img');
    const imgNova = document.createElement('img');

    const classeBranco = `${nome}Branco`;
    const classePreto = nome;


    imgNova.src = `${isBranco ? `newPecas/branca/${pecaBranco}` : `newPecas/preta/${pecaPreta}`}`;
    imgNova.classList.add('peca', isBranco ? classeBranco : classePreto);
    ...
    pecaCliclada.classList.remove(classeBranco, classePreto);
    ...
    cell.classList.add(isBranco ? classeBranco : classePreto);
